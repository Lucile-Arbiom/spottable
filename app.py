...
# 2. Style CSS (Design harmonisé avec vos couleurs)
st.markdown(f"""
    <style>
...
# Fonction pour extraire les coordonnées précises d'un lien Google Maps
def extract_coords(url):
    if pd.isna(url): return None, None
    # Cherche le pattern @lat,lon dans l'URL
...
try:
    ...
    # Tentative d'affinage de la précision via la colonne de géolocalisation
    c_link = next((c for c in df.columns if any(w in c.lower() for w in ['map', 'lien', 'geo'])), None)
    
    if c_link:
        # On extrait les coordonnées de l'URL pour écraser les lat/lon moins précises
        df['coords'] = df[c_link].apply(extract_coords)
        df['extracted_lat'] = df['coords'].apply(lambda x: x[0])
        df['extracted_lon'] = df['coords'].apply(lambda x: x[1])
        
        # On remplace lat/lon par les versions extraites si elles existent
        df['lat'] = df['extracted_lat'].fillna(pd.to_numeric(df.get('latitude', df.get('lat')), errors='coerce'))
        df['lon'] = df['extracted_lon'].fillna(pd.to_numeric(df.get('longitude', df.get('lon')), errors='coerce'))
    else:
