...
# 2. Style CSS complet (Optimisé pour forcer les couleurs)
st.markdown(f"""
    <style>
...
try:
    ...
    with col1:
        ...
        if not df_map.empty:
            ...
            if user_layer: layers.append(user_layer)

            view_state = pdk.ViewState(latitude=df_map["lat"].mean(), longitude=df_map["lon"].mean(), zoom=12)
